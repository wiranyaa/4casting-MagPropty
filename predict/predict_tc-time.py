...
def auto_mode(session, output_dir):
    ...
    for thickness in thicknesses:
        for diameter in diameters:
            ...
            filename = os.path.join(output_dir, f"tc-time_t{thickness}d{diameter}.csv")
            ...
            try:
                ...
                # Add average prediction time to the plot
                plt.text(
                    0.7 * max(temperatures), 0.8 * max(magnetization_smooth),
                    f'Avg Time: {avg_prediction_time:.4f} sec',
                    fontsize=15, color='blue', bbox=dict(facecolor='white', alpha=0.5)
                )
            
            except Exception as e:
                print(f"Error in fitting data: {e}")

            # Customize plot for each pair
            plt.xlabel('Temperature (K)', fontsize=20)
            plt.ylabel(r'Magnetization (M/M$\mathregular{_s}$)', fontsize=20)
            plt.xticks(fontsize=20)
            plt.yticks(fontsize=20)
            plt.title(f'Predicted Curie Temperature for {label}', fontsize=20)
            plt.legend(fontsize=17)
            plt.xlim(left=0)
            plt.ylim(bottom=0)

            # Save each plot to a separate PNG file
            plot_filename = os.path.join(output_dir, f"tc-time_t{thickness}_d{diameter}.png")
