...
def combine_files(output_dir):
    ...
    combined_df.to_csv(os.path.join(output_dir, "all-Thickness-data.csv"), index=False)
    print("All processed files combined into all-Thickness-data.csv")
...
def train_and_convert_model(X_train, X_test, y_train, y_test, output_dir):
    ...
    large_rf_model = RandomForestRegressor(n_estimators=100, random_state=42)
    large_rf_model.fit(X_train, y_train)
    
    joblib_path = os.path.join(output_dir, 'large_model.joblib')
    onnx_path = os.path.join(output_dir, 'large_model.onnx')
    
    joblib.dump(large_rf_model, joblib_path)
    
    initial_type = [('float_input', FloatTensorType([None, 3]))]
    # เพิ่มค่า opset ให้ใหม่ขึ้นเพื่อเพิ่มความแม่นยำ
    onx = convert_sklearn(large_rf_model, initial_types=initial_type, target_opset=14)
    ...
    return large_rf_model, compute_time

def compare_predictions(large_rf_model, X_test, output_dir):
    sklearn_pred = large_rf_model.predict(X_test)
...
def main():
    ...
    print("Loading data...")
    data = pd.read_csv(os.path.join(output_dir, 'all-Thickness-data.csv'))
    ...
    large_rf_model, compute_time = train_and_convert_model(X_train, X_test, y_train, y_test, output_dir)
    print(f"Training and conversion took: {compute_time:.2f} seconds")
    
    print("Comparing predictions...")
    compare_predictions(large_rf_model, X_test, output_dir)
