...
def check_model_file():
    ...
    model_path = os.path.join(current_directory, "small_model.onnx")
...
def auto_mode(session, output_dir):
    ...
    for thickness in thicknesses:
        for diameter in diameters:
            ...
            # Save data to CSV
            filename = os.path.join(output_dir, f"small_model-data_t{thickness}d{diameter}.csv")
            ...
            plt.grid(True)
            plt.savefig(os.path.join(output_dir, f"small_model-data_t{thickness}d{diameter}.png"))
            plt.show()
